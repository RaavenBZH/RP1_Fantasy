...
def input() -> list:
    ...
    theFile = open('RP1_Fantasy\s6\entrees_rp1.csv')
...
def output(classement : dict, equipes : dict = {}) -> None:
    ...
    f = open('RP1_Fantasy\s6\sorties_rp1.csv', 'w', newline="")
    writer = csv.writer(f)
...
def generateRound(roundNum : int, track : str = "") -> None:
    ...
    if roundNum < 1:
        ...
        w("\n")
        w(f"result{num} = GrandPrix(\"{track}\")\n")
        w("\n")
        w(f"qualif{num} = [\n]\n")
        w("\n")
        w(f"course{num} = [\n]\n")
        ...
        w(f"result{num}.resultat(\"c\", course{num})\n")
        w("\n")
